...
define('INSTALL_DIR', '/var/www/html/');
define('SETUP_DIR', '/var/www/src/setup/'); // use git setup
...
define('CRON_JOB_SCRIPT', '/var/www/html/api/cron.php');
...
$vars = [
    ...
    'smtp_tls_certs' =>
        getenv('SMTP_TLS_CERTS') ?: '/etc/ssl/certs/ca-certificates.crt',
...
$mailConfig = str_replace(
    '%SMTP_TLS%',
    boolToOnOff(convertStrToBool('smtp_tls', true)),
    $mailConfig,
);
$mailConfig = str_replace(
    '%SMTP_AUTH%',
    boolToOnOff($vars['smtp_user'] != ''),
    $mailConfig,
);

if (
    !file_put_contents(MAIL_CONFIG_FILE, $mailConfig) ||
    !chown(MAIL_CONFIG_FILE, 'www-data') ||
    !chgrp(MAIL_CONFIG_FILE, 'www-data') ||
    !chmod(MAIL_CONFIG_FILE, 0600)
) {
...
if ($interval > 0) {
    ...
    $cron =
        "*/{$interval} * * * * www-data /usr/bin/php " . CRON_JOB_SCRIPT . "\n";
...
$t = 0;
while (
    !@socket_connect($s, $vars['dbhost'], 3306) &&
    $t < CONNECTION_TIMEOUT_SEC
) {
    $t++;
    if ($t % 15 == 0) {
...
if (!db_connect($vars['dbhost'], $vars['dbuser'], $vars['dbpass'])) {
    err(
        sprintf(
            __('Unable to connect to MySQL server: %s'),
            db_connect_error(),
        ),
    );
} elseif (
    explode('.', db_version()) < explode('.', $installer->getMySQLVersion())
) {
    err(
        sprintf(
            __('osTicket requires MySQL %s or later!'),
            $installer->getMySQLVersion(),
        ),
    );
} elseif (
    !db_select_database($vars['dbname']) &&
...
$sql =
    'INSERT INTO `' .
    $vars['prefix'] .
    "config` (`id`, `namespace`, `key`, `value`, `updated`) VALUES (NULL, 'core', 'system_language', '" .
    LANGUAGE .
    "', CURRENT_TIMESTAMP);";
if (db_query($sql, false)) {
